...
class Encoder(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, pe_max_len,name,
        ...
        if (self.acous_seman is not None):
            ...
            self.dropout = tf.keras.layers.Dropout(rate=self.rate, name='en_proj_dp')

        self.enc_layers = [EncoderLayer(d_model, num_heads, dff, 'EN'+str(_),dp) for _ in range(num_layers)]
