...
loss_object_acoustic = tf.keras.losses.SparseCategoricalCrossentropy(
    from_logits=True, reduction='none')
loss_object_semantic = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
...
def Loss(real,pred):
    mask = tf.math.logical_not(tf.math.equal(real, 0))
    loss_ = loss_object_semantic(real, pred)
...
def loss_function(real, pred):
  mask = tf.math.logical_not(tf.math.equal(real, 0))
  loss_ = loss_object_acoustic(real, pred)
...
if __name__=='__main__':
    ...
    real = tf.convert_to_tensor([2,1,0], tf.int64)
